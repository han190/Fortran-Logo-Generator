...
subroutine read_formatted(logo, unit, iotype, v_list, iostat, iomsg)
  ...
  !> Avoid gfortran warning.
  associate (v_list_ => v_list, &
    & iostat_ => iostat, iomsg_ => iomsg)
  end associate
...
subroutine draw(logo)
  ...
  call svg%write_prolog([ &
    & 'version'.pair.'1.0', &
    & 'encoding'.pair.'utf-8', &
    & 'standalone'.pair.'no'])

  !> Determine offset
  call svg%write_attribute('svg', [ &
  ...
  call svg%write_element('desc', 'Fortran Logo Generator')

  !> Fill Pattern
  call svg%write_attribute('pattern', [ &
  ...
  !> Compare with other logo (debug)
  if (logo%compare) then
    call svg%write_attribute('image', [ &
      & 'href'.pair.logo%compare_image, &
      & 'x'.pair.0.5*logo%width*(logo%canvas_ratio - 1), &
      & 'y'.pair.0.5*logo%height*(logo%canvas_ratio - 1), &
  ...
  associate ( &
    ...
    if (logo%pile_radius > 0.) then
      ...
      !> Uncomment to draw centroid of "F"
...
contains
  ...
  subroutine circle(point)
    ...
    call svg%write_attribute('circle', [ &
      & 'fill'.pair.'none', &
      & 'stroke'.pair.logo%color, &
      & 'cx'.pair.converted%x, &
      & 'cy'.pair.converted%y, &
      & 'r'.pair.logo%pile_radius])
